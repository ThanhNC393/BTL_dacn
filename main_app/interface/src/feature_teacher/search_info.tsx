import { useState, useEffect } from "react";
import "bootstrap/dist/css/bootstrap.min.css";
import api from "../apis";

interface PersonInfo {
  address: string;
  date_of_joining: string;
  email: string;
  name: string;
  personal_id: string;
  phone_number: string;
  school_id: string;
  class_name?: string;
}

interface Schedule {
  [date: string]: Array<{ [subject: string]: number[] }>;
}

interface StudentResult {
  [subject: string]: {
    scores: (number | null)[];
    scores_name: string[];
  };
}

const DAYS = ["Thứ 2", "Thứ 3", "Thứ 4", "Thứ 5", "Thứ 6", "Thứ 7", "Chủ nhật"];

export default function SearchInfo() {
  const [mode, setMode] = useState<"student" | "teacher">("student");
  const [list, setList] = useState<Record<string, PersonInfo>>({});
  const [selected, setSelected] = useState("");
  const [info, setInfo] = useState<PersonInfo | null>(null);
  const [schedule, setSchedule] = useState<Schedule>({});
  const [weekOffset, setWeekOffset] = useState(0);

  const [subjects, setSubjects] = useState<string[]>([]);
  const [results, setResults] = useState<StudentResult>({});
  const [search, setSearch] = useState("");

  /* ================= FETCH LIST ================= */
  const fetchList = async () => {
    try {
      const url = mode === "teacher" ? "/get_teachers" : "/get_students";
      const res = await api.post(url, {});
      setList(res.data || {});
    } catch (err) {
      console.error("fetchList error", err);
      setList({});
    }
  };

  const filteredList = Object.fromEntries(
    Object.entries(list).filter(([id]) =>
      id.toLowerCase().includes(search.toLowerCase())
    )
  );

  const searchById = () => {
    if (!search) return;
    if (!list[search]) {
      alert("Không tìm thấy mã trong danh sách!");
      return;
    }
    setSelected(search);
    fetchInfo(search);
  };

  /* ================= FETCH INFO ================= */
  const fetchInfo = async (id: string) => {
    if (!id) return;

    setInfo(list[id] || null);
    setSchedule({});
    setSubjects([]);
    setResults({});

    try {
      // timetable
      const tkbUrl =
        mode === "teacher" ? "/get_tkb/teacher" : "/get_tkb/student";
      const tkbRes = await api.post(tkbUrl, { id });
      setSchedule(tkbRes.data || {});
    } catch {
      setSchedule({});
    }

    try {
      // subjects
      const subjUrl =
        mode === "teacher"
          ? "/get_subject_of/teacher"
          : "/get_subject_of/student";
      const subjRes = await api.post(subjUrl, [id]);
      setSubjects(subjRes.data || []);

      // student results
      if (mode === "student") {
        const resRes = await api.post("/get_result/student", [id]);
        setResults(resRes.data || {});
      }
    } catch {
      setSubjects([]);
      setResults({});
    }
  };

  useEffect(() => {
    setSelected("");
    setInfo(null);
    setSchedule({});
    setSubjects([]);
    setResults({});
    setWeekOffset(0);
    fetchList();
  }, [mode]);

  /* ================= WEEK ================= */
  const getWeekStart = (): Date => {
    const now = new Date();
    const day = now.getDay();
    const diff = day === 0 ? -6 : 1 - day;
    const monday = new Date(now);
    monday.setDate(now.getDate() + diff + weekOffset * 7);
    monday.setHours(0, 0, 0, 0);
    return monday;
  };

  const getWeekEnd = (start: Date): Date => {
    const end = new Date(start);
    end.setDate(start.getDate() + 6);
    end.setHours(23, 59, 59, 999);
    return end;
  };

  /* ================= TIMETABLE ================= */
  const renderTimetable = () => {
    const weekStart = getWeekStart();
    const weekEnd = getWeekEnd(weekStart);

    const timetable: { [day: string]: { [p: number]: string[] } } = {};
    DAYS.forEach((d) => (timetable[d] = {}));

    Object.entries(schedule).forEach(([dateStr, subs]) => {
      const date = new Date(dateStr + "T00:00:00");
      if (date < weekStart || date > weekEnd) return;

      const idx = date.getDay() === 0 ? 6 : date.getDay() - 1;
      const dayName = DAYS[idx];

      subs.forEach((obj) =>
        Object.entries(obj).forEach(([s, ps]) =>
          ps.forEach((p) => {
            if (!timetable[dayName][p]) timetable[dayName][p] = [];
            if (!timetable[dayName][p].includes(s))
              timetable[dayName][p].push(s);
          })
        )
      );
    });

    return (
      <>
        <div className="d-flex justify-content-between mb-2">
          <button
            className="btn btn-outline-secondary"
            onClick={() => setWeekOffset((w) => w - 1)}
          >
            ◀ Tuần trước
          </button>
          <strong>
            {getWeekStart().toLocaleDateString()} –{" "}
            {getWeekEnd(getWeekStart()).toLocaleDateString()}
          </strong>
          <button
            className="btn btn-outline-secondary"
            onClick={() => setWeekOffset((w) => w + 1)}
          >
            Tuần sau ▶
          </button>
        </div>

        <table className="table table-bordered text-center">
          <thead>
            <tr>
              <th>Tiết</th>
              {DAYS.map((d) => (
                <th key={d}>{d}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {Array.from({ length: 20 }, (_, i) => i + 1).map((p) => (
              <tr key={`p-${p}`}>
                <td>{p}</td>
                {DAYS.map((d) => (
                  <td key={`${d}-${p}`}>
                    {(timetable[d][p] || []).map((s) => (
                      <div key={`${d}-${p}-${s}`}>{s}</div>
                    ))}
                  </td>
                ))}
              </tr>
            ))}
          </tbody>
        </table>
      </>
    );
  };

  /* ================= SUBJECT / RESULT ================= */
  const renderSubjectsAndResults = () => {
    if (!selected) return null;

    if (mode === "teacher") {
      return (
        <div className="card p-3 mt-4">
          <h5>Môn giảng dạy</h5>
          <table className="table table-bordered text-center">
            <tbody>
              {subjects.map((s, i) => (
                <tr key={`${s}-${i}`}>
                  <td>{i + 1}</td>
                  <td>{s}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      );
    }

    if (!subjects.length) return null;
    const cols = results[subjects[0]]?.scores_name || [];

    return (
      <div className="card p-3 mt-4">
        <h5>Kết quả học tập</h5>
        <table className="table table-bordered text-center">
          <thead>
            <tr>
              <th>Môn</th>
              {cols.map((c, i) => (
                <th key={`${c}-${i}`}>{c}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {subjects.map((s) => (
              <tr key={s}>
                <td>{s}</td>
                {cols.map((_, i) => (
                  <td key={`${s}-${i}`}>
                    {results[s]?.scores[i] ?? "Chưa có"}
                  </td>
                ))}
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    );
  };

  /* ================= UI ================= */
  return (
    <div className="container mt-4">
      <h3>Tra cứu thông tin</h3>

      <div className="mb-3">
        <button
          className={`btn me-2 ${
            mode === "student" ? "btn-primary" : "btn-outline-primary"
          }`}
          onClick={() => setMode("student")}
        >
          Sinh viên
        </button>
        <button
          className={`btn ${
            mode === "teacher" ? "btn-primary" : "btn-outline-primary"
          }`}
          onClick={() => setMode("teacher")}
        >
          Giảng viên
        </button>
      </div>

      <input
        className="form-control mb-2"
        placeholder="Nhập mã"
        value={search}
        onChange={(e) => setSearch(e.target.value)}
      />
      <button className="btn btn-success w-100 mb-2" onClick={searchById}>
        Tìm
      </button>

      <select
        className="form-select"
        value={selected}
        onChange={(e) => {
          setSelected(e.target.value);
          fetchInfo(e.target.value);
        }}
      >
        <option value="">-- Chọn --</option>
        {Object.keys(filteredList).map((id) => (
          <option key={id} value={id}>
            {id} - {filteredList[id].name}
          </option>
        ))}
      </select>

      {info && (
        <div className="card p-3 mt-4">
          <h5>Thông tin cá nhân</h5>
          <p>Họ tên: {info.name}</p>
          <p>Mã: {info.school_id}</p>
          <p>Email: {info.email}</p>
          {info.class_name && <p>Lớp: {info.class_name}</p>}
        </div>
      )}

      {Object.keys(schedule).length > 0 && renderTimetable()}
      {renderSubjectsAndResults()}
    </div>
  );
}
